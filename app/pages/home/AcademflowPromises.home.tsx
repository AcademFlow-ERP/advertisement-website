...
export default function AcademflowPromisesHome(){
    return(
        <section className="w-full relative flex flex-col items-center justify-center p-15 text-[#31859c] bg-[#f6f6f6]">
            <div className="w-full flex flex-col justify-center items-center max-w-7xl px-5 py-5 gap-10">
                <div className="flex flex-col gap-5">
                    <h1 className="text-3xl sm:text-4xl max-w-4xl font-poppins font-bold text-center">The Academflow Promise.</h1> 
                    <h1 className="text-2xl max-w-2xl font-poppins font-bold text-gray-900 text-center">Four Ways in which Academflow is <span>inspiring better education management solution - </span> Our Promise</h1> 
                </div>
                <div className="w-full flex flex-row justify-center flex-wrap gap-5 max-w-4xl">
                    {
                        academflowPromisesCollection.map(({id, title, name}, index) => (
                            <div className="w-full flex flex-1 min-w-[20rem] max-w-sm gap-5 items-center text-white bg-[#31859c] p-5 rounded-4xl" key={index+id}>
